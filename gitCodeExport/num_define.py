...
class Expression:
    ...
    def forr(self, names):
        ...
        self.name = self.last_char_in(names[0:names.index(" ")])
        #self.name = names[0:names.index(" ")]
        
        #get vowels and consonants
        vowels = [ord(char.lower())-96 for char in self.name if char.lower() in 'aeiou']
        consonants = self.name.translate(str.maketrans('','','aeiouAEIOU'))
        
        #consonants to num
        cons_to_num = [ord(char.lower())-96 for char in self.name if char.lower() in consonants.lower()]
        ...
        #check for Y in name and put in list b4 suming
        #self.last_char_in(self.names)
...
#exp = Expression()
#print(exp.forr('karen ')) #Princely Okwuego Emeka
